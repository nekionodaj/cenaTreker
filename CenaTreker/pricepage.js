...
function printPrice(tab) {
  ...
  Object.keys(bg).forEach(function (url) { //za svaki kljuc u bg(u biti je to window.data) vrsimo istu funkciju koju smo prije za bg.data
    ...
    <div class="input-group-append">
    <a href='#' target="_blank" id="gumbLink"><button id='zutiGumb' class="btn btn-info" data-toggle="tooltip" data-placement="right" title="press to visit item on Amazon">Visit</button></a>
    <button id="makni" class="btn btn-outline-danger" data-toggle="tooltip" data-placement="right" title="press to remove the item from the list">Remove</button>
    </div></div>`);
